...
def get_answers(name):
    ...
    for ids in goldsetids:
        ...
        for index, row in dataframe.iterrows():
            ...
            candidate_question = row.CandidateQuestion
            relevant_candidate_answer = row.RelevantAnswerDetail
            prompt = str(candidate_question) + str(relevant_candidate_answer) + str(question) + "\n ## Answer : \n "
            sequences = pipe( prompt, do_sample=True, max_new_tokens=100, temperature=0.7, top_k=50, top_p=0.95, num_return_sequences=1,)
            dataframe.at[index, 'MistralAnswer'] = sequences[0]['generated_text']
        
        base_filename, extension = os.path.splitext(name)
        name_path = ""+base_filename+'.csv'
