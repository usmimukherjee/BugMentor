...
# from utils.Entity.Entity_Analysis import get_entity_score, get_entities_from_word_list
...
def get_querylist(list_path):
    filr = open(list_path, encoding='utf-8', errors='ignore')
    query_list = []
    i = 0
    for line in filr:
        if i % 2 == 1:
...
def preprocess_all_questions(questions, idf, w2v, stopword):
    ...
    for question in questions:
        ...
        question.title = title_words
        question.matrix = init_doc_matrix(question.title, w2v)
        question.idf_vector = init_doc_idf_vector(question.title, idf)
...
if __name__ == '__main__':
    filenames = ["angular", "ansible", "atom", "axios", "dolphinscheduler", "elasticsearch", "electron", "godotengine",
                 "golang", "huggingface", "keras", "kubernetes", "kubernetesclient", "lighthouse", "moby", "nodejs",
                 "polymer", "react", "rust", "saltstack", "sympy", "tensorflow", "typescript", "vscode"]
    for name in filenames:
        df = pd.read_csv("E:\Pycharm Projects\Thesis - QA\AnswerBot\Gold\\" + filename + "_goldset.csv")
